...
func Test_addToPathFront(t *testing.T) {
	...
	tests := []struct {
        ...
        { 
            ...
            want: "/Users/stephen.reilly/.local/bin:/usr/bin:/usr/local/bin",
            wantErr: false,
        },
        { 
...
func Test_addToPathBack(t *testing.T) {
    t.Setenv("PATH", "/usr/bin:/usr/local/bin")
	tests := []struct {
		name string // description of this test case
		// Named input parameters for target function.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, gotErr := addToPathBack(tt.dir)
			if gotErr != nil {
				if !tt.wantErr {
					t.Errorf("addToPathBack() failed: %v", gotErr)
				}
				return
			}
			if tt.wantErr {
				t.Fatal("addToPathBack() succeeded unexpectedly")
			}
			// TODO: update the condition below to compare got with tt.want.
            if got != tt.want {
				t.Errorf("addToPathFront() = %v, want %v", got, tt.want)
